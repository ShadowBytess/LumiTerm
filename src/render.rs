    ...
    pub fn render(&mut self, term: &Term, buffer: &mut [u32], width: usize, height: usize, padding: usize) {
        ...
        for y in 0..term.rows {
            for x in 0..term.cols {
                let cell = term.grid[y * term.cols + x];
                ...
                if cell.bg != term.default_bg {
                    let bgpx = pack(cell.bg[0], cell.bg[1], cell.bg[2]);
                    for yy in 0..cell_h {
                ...
                if cell.ch != ' ' && cell.ch != '\0' {
                    ...
                    for gy in 0..metrics.height {
                        for gx in 0..metrics.width {
                            ...
                            let bg_here = if cell.bg != term.default_bg { cell.bg } else { term.default_bg };
                            let blended = blend(fg, bg_here, coverage);
